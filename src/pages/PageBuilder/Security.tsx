...
const Security = ({ mainData }: any) => {
    ...
    const initialValues = {
        Security_title:(mainData?.map((item: any) => item?.Security_title).join('') || '').replace(/,/g, ''),
        Security_body: (mainData?.map((item: any) => item?.Security_body).join('') || '').replace(/,/g, ''),
    };
        ...
        <>
            <div className="lg:col-span-12 max-sm:col-span-1 px-1 ">
                <div className="flex flex-row items-center justify-between w-[100%]  border py-3 px-2  bg-gray-100 ">
                    <h6 className=" font-bold ">Security Section</h6>
                    <button onClick={open}>
            ...
            <Modal opened={opened} size="50% " onClose={close} title="Security Section">
                <Formik
                    ...
                    onSubmit={(values) => {
                        ...
                        update({ ...values, page:"Security_page"  });
                    }}
                >
                    <Form>
                        <div className=" grid grid-cols-12 gap-2">
                            <div className="col-span-12">
                                ...
                                <InputCustom name="Security_title" label="title" />
                            </div>

                            <div className="col-span-12">
                                <label htmlFor="title">Description</label>
                                <TextAreaField name="Security_body" label="sub title" />
                            </div>
