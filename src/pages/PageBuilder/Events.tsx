...
const Events = ({ mainData }: any) => {
    ...
    const initialValues = {
        Events_title: (mainData?.map((item: any) => item?.Events_title).join('') || '').replace(/,/g, ''),
        Events_body: (mainData?.map((item: any) => item?.Events_body).join('') || '').replace(/,/g, ''),
    };
        ...
        <>
            <div className="lg:col-span-12 max-sm:col-span-1 px-1 ">
                <div className="flex flex-row items-center justify-between w-[100%]  border py-3 px-2  bg-gray-100 ">
                    <h6 className=" font-bold ">Events Section</h6>
                    <button onClick={open}>
                        <IconEdit className="text-blue-700" />
                    </button>
                </div>
            </div>
            <Modal opened={opened} size="50% " onClose={close} title="Events">
                <Formik
                    ...
                    onSubmit={(values) => {
                        ...
                        update({ ...values , page:"Events_page"  });
                    }}
                >
                    <Form>
                        <div className=" grid grid-cols-12 gap-2">
                            <div className="col-span-12">
                                ...
                                <InputCustom name="Events_title" label="title" />
                            </div>
                            <div className="col-span-12">
                                <label htmlFor="title">Description</label>
                                <TextAreaField name="Events_body" label="sub title" />
                            </div>
